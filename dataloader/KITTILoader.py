...
class KITTILoader(object):
    ...
    def __init__(self, config={}):
        ...
        if not root_path.is_absolute():
            root_path = (Path(__file__).resolve().parent.parent / root_path).resolve()
        ...
        if isinstance(max_frames, int) and max_frames > 0:
            self.img_N = min(self.img_N, self.config["start"] + max_frames)
    ...
    def __getitem__(self, item):
        ...
        if img is None:
            raise FileNotFoundError(f"Failed to read image: {file_name}")
        return img
