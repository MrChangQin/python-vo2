...
def parse_arguments():
    ...
    parser.add_argument('--synthetic_root_path', type=str, default='../dataset/coco_20k',
    ...
    parser.add_argument('--n_steps', type=int, default=160_000,  # 160_000
...
class Trainer():
    """
        Class for training XFeat with default config as described in the paper.
    ...
    def __init__(self, megadepth_root_path,
                 synthetic_root_path,
                 ckpt_save_path,
                 model_name='xfeat_default',  # xfeat_default, xfeat_synthetic, xfeat_megadepth
        ...
        self.steps = n_steps
        self.opt = optim.Adam(filter(lambda x: x.requires_grad, self.net.parameters()), lr=lr)  # Adam优化器，用于更新模型参数
        self.scheduler = torch.optim.lr_scheduler.StepLR(self.opt, step_size=30_000,
                                                         gamma=gamma_steplr)  # 学习率调度器，每30000步降低学习率

        ##################### Synthetic COCO INIT ##########################
        if model_name in ('xfeat_default', 'xfeat_synthetic'):
            self.augmentor = AugmentationPipe(  # 数据增强管道，用于生成合成图像对
        ...
        if model_name in ('xfeat_default', 'xfeat_megadepth'):
            ...
            data = torch.utils.data.ConcatDataset([MegaDepthDataset(root_dir=TRAINVAL_DATA_SOURCE,
                                                                    npz_path=path) for path in
                                                   tqdm.tqdm(npz_paths, desc="[MegaDepth] Loading metadata")])

            self.data_loader = DataLoader(data,
                                          batch_size=int(
                                              self.batch_size * 0.6 if model_name == 'xfeat_default' else batch_size),
                                          shuffle=True)
        ...
        with tqdm.tqdm(total=self.steps) as pbar:
            for i in range(self.steps):
                ...
                for b in range(len(positives_c)):
                    ...
                    #Compute losses
                    loss_ds, conf = dual_softmax_loss(m1, m2)  # 计算双软最大损失和置信度
                    loss_coords, acc_coords = coordinate_classification_loss(coords1, pts1, pts2, conf)  # 计算坐标分类损失和准确率

                    loss_kp_pos1, acc_pos1 = alike_distill_loss(kpts1[b], p1[b])  # 计算关键点位置损失和准确率
                    loss_kp_pos2, acc_pos2 = alike_distill_loss(kpts2[b], p2[b])  # 计算关键点位置损失和准确率
                    loss_kp_pos = (loss_kp_pos1 + loss_kp_pos2) * 2.0
                    acc_pos = (acc_pos1 + acc_pos2) / 2

                    loss_kp = keypoint_loss(h1, conf) + keypoint_loss(h2, conf)  # 计算关键点损失
                ...
                #         loss.item(), acc_coarse_0, acc_coarse, acc_coords, loss_coarse, loss_coord, loss_l1, nb_coarse,
                #         loss_kp_pos, acc_pos))
                
                pbar.set_description(
                    "\033[1m\033[91m[Loss]: {loss:.4f}\033[0m "                          # 🔴 总损失
                    "\033[1m\033[92m[Coarse] acc_Syn={acc_c0:.3f} acc_Mega={acc_c1:.3f} loss={loss_c:.3f}\033[0m "  # 🟢 合成数据粗匹配：acc → loss
                    "\033[1m\033[96m[Fine] acc_Mega={acc_f:.3f} loss={loss_f:.3f}\033[0m "           # 🟣 精匹配：acc → loss
                    "\033[1m\033[93m[KPs] acc_pos={acc_kp_pos:.3f} loss_pos={loss_kp_pos:.3f} loss_rel={loss_kp:.3f}\033[0m "  # 🟡 关键点：acc_pos → loss_pos → loss_kp
                    "\033[1m\033[95m[Count]: {nb_matches:d}\033[0m"                              # 🟠 匹配数量
                    .format(
                        loss=loss.item(),
                        acc_c0=acc_coarse_0,        # 🟢 合成数据粗匹配准确率
                        acc_c1=acc_coarse,          # 🔵 MDepth粗匹配准确率
                        loss_c=loss_coarse,
                        acc_f=acc_coords,
                        loss_f=loss_coord,
                        loss_kp=loss_l1,
                        loss_kp_pos=loss_kp_pos,
                        acc_kp_pos=acc_pos,
                        nb_matches=nb_coarse
                    )
                )
                pbar.update(1)

                # Log metrics
                self.writer.add_scalar('Loss/total', loss.item(), i)  # 记录总损失值
                self.writer.add_scalar('Accuracy/coarse_synth', acc_coarse_0, i)  # 记录粗匹配准确率
                self.writer.add_scalar('Accuracy/coarse_mdepth', acc_coarse, i)  # 记录粗匹配准确率
                self.writer.add_scalar('Accuracy/fine_mdepth', acc_coords, i)  # 记录精匹配准确率
                self.writer.add_scalar('Accuracy/kp_position', acc_pos, i)  # 记录关键点位置准确率
                self.writer.add_scalar('Loss/coarse', loss_coarse, i)  # 记录粗匹配损失值
                self.writer.add_scalar('Loss/fine', loss_coord, i)  # 记录精匹配损失值
                self.writer.add_scalar('Loss/reliability', loss_l1, i)  # 记录可靠性损失值
                self.writer.add_scalar('Loss/keypoint_pos', loss_kp_pos, i)  # 记录关键点位置损失值
                self.writer.add_scalar('Count/matches_coarse', nb_coarse, i)  # 记录粗匹配的匹配点数量
