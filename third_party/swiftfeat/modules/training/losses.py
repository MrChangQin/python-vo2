...
# dual_softmax_loss 是一个用于计算特征匹配的双向softmax损失函数。
# 它的主要作用是评估两组特征之间的匹配质量。例如，在图像匹配任务中，
# 它可以用来判断图片A中的关键点是否与图片B中的关键点正确匹配。
# 输入：
    # X: 特征向量，形状为 [N, C]，其中 N 是特征向量的数量，C 是特征向量的维度。
    # Y: 特征向量，形状为 [N, C]，其中 N 是特征向量的数量，C 是特征向量的维度。
    # temp: 温度参数，用于控制 softmax 的温度。
# 输出：
    # loss: 损失值，用于衡量特征匹配的质量。
    # conf: 置信度，用于评估特征匹配的质量。
